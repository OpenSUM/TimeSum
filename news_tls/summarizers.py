...
class SubmodularSummarizer(Summarizer):
    ...
    """
    def __init__(self, a=5, div_weight=6,con_weight=0.1,imp_weight=0.05, cluster_factor=0.2):
    ...
    def compute_summary_consistance(self,
                                 alpha,
                                 summary_indices,
                                 sent_coverages,
                                 pairwise_sims):
        ...
        for i in summary_indices:
            ...
            try:
                ...
                for each in ngram_vectorizer1.vocabulary_:
                    ...
                    tsum+=(ngram_freq[index] if index else 0 )
            except ValueError:
                pass
        return tsum

    def oracle(self,summary_indices,ground_truth,sents,d):
        timeline=[]
        summary=[sents[i].raw for i in summary_indices]
        time = datetime.datetime(d.year, d.month, d.day)
        timeline.append((time, summary))
        timeline.sort(key=lambda x: x[0])
        TL=data.Timeline(timeline)
        pred_timeline = TilseTimeline(TL.date_to_summaries)
        tt=rouge.TimelineRougeEvaluator(measures=["rouge_1"]).evaluate_align_date_content_costs_many_to_one(pred_timeline, ground_truth)
        return tt['rouge_1']['f_score']
    ...
    def optimise(self,
                 ...
                 sent_coverages,
                 avg_sent_sims,
                 x1,ngram_vectorizer,ngram_freq,ref,d):

        alpha = self.a / len(sents)
        remaining = set(range(len(sents)))
        selected = []
        #ground_truth = TilseGroundTruth([ref])
        while len(remaining) > 0 and len(selected) < k:
            i_to_score = {}
            for i in remaining:
                ...
                con = self.compute_summary_consistance(
                    alpha, summary_indices, sent_coverages, pairwise_sims)
                imp = self.compute_summary_importance(summary_indices,x1,sents,ngram_vectorizer,ngram_freq)
                #self.compute_summary_importance(summary_indices,x1,sents,ngram_vectorizer,ngram_freq)
                #oracle = self.oracle(summary_indices,ground_truth,sents,d)
                score = cov + self.div_weight * div +self.imp_weight*imp
                i_to_score[i] = score
            ranked = sorted(i_to_score.items(), key=lambda x: x[1], reverse=True)
            for i, score in ranked:
                s = sents[i]
    ...
    def summarize(self, sents, k, vectorizer, filter=None,ref=None,dt=None):
        raw_sents = [s.raw for s in sents]
        ...
        selected = self.optimise(
            ...
            pairwise_sims, sent_coverages, avg_sent_sims,x1,ngram_vectorizer,ngram_freq,ref,d=dt
        )
