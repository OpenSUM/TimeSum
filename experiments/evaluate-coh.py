        ...
        for j, ref_timeline in enumerate(ref_timelines):
            ...
            if (n_ref>=0):
                ...
                for date in sorted(pred_timeline.dates_to_summaries.keys()):
                    ...
                    pairwise_sims=0
                    if(tx1!=[]):
                        x1 = vec.fit_transform(tx1)
                        x1 = x1.toarray().sum(axis=0)
                        x2 = vec.fit_transform(tx2)
                        x2 = x2.toarray().sum(axis=0)
                        a=min(len(x1),len(x2))
                        x1=x1[:a]
                        x2=x2[:a]
                        x1=x1.reshape(1,-1)
                        x2=x2.reshape(1,-1)
                        pairwise_sims = cosine_similarity(x1,x2)
                    tx1=tx2
                    datelist.append(pairwise_sims)
                    f = open(f'out-coh-{topic}-{j}.txt', "w")
